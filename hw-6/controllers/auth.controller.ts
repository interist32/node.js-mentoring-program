...
import { HTTP_ERROR, JWT_SECRET, JWT_EXPIRATION_TIME_MS } from '../constants/http-errors.enum';
...
export const login = ({ body }: Request, res: Response): void => {
  ...
  if (isValidUser(username, password)) {
    ...
    const token = jwt.sign(payload, JWT_SECRET, { expiresIn: JWT_EXPIRATION_TIME_MS });
    res.send(token);
  } else {
